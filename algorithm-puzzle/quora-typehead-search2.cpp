...
inline bool pre(string& sub, int s){
    if (strlen(data+s) < sub.length()) return false;
    for (ui i = 0; i < sub.length(); i++)
        if (data[s+i] != sub[i]) return false;
    return true;
}
inline int cmp(string& w, int s){
    const char* s1 = w.c_str();
    return strcmp(s1, data+s);
}
inline bool prefix(string& word, vector<int>& tokens){
    int L = 0, R = tokens.size() - 1, M;
    while(L <= R){
        M = (L+R) >> 1;
        if (pre(word, tokens[M])) return true;
        else{
            if (cmp(word, tokens[M]) > 0) L = M+1;
            else R = M-1;
        }
    }
    return false;
}
bool check(vector<string>& words, vector<int>& tokens){
