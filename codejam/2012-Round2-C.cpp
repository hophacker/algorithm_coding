...
int height[2002];
int peak[2002];
bool go(int L, int R, int slop){
    int i = L;
    while(true){
        int j = peak[i];

        height[i] = height[R]- slop * (R-i);
        if (height[j] == -1) 
            height[j] = height[R] - slop * (R-j);

        if (i+1 < j){
            if (!go(i+1, j, slop+1)) return false;
        }
...
int main ( int argc, char *argv[] ) {
    ...
    FE(cases,1,T){
        wez(N);
        FE(i,1,N-1) getI(peak[i]);
        clr(height, -1);
        height[N] = maxH;
        printf("Case #%d:", cases);
        if (go(1, N, 0)){
            FE(i,1,N) printf(" %d", height[i]);
            printf("\n");
