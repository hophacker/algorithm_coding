...
struct node{
    double d, w;
    node(double dd, double ww):d(dd), w(ww){
...
int main ( int argc, char *argv[] ) {
    ...
    FE(cases,1,T){
        ...
        F(i,0,N){
            wez3(b, e, w);
            if (b > lastE) seg.push_back(node(b - lastE, S));
            seg.push_back(node(e - b, w+S));
            lastE = e;
        }
        if (lastE < X) seg.push_back(node(X - lastE, S));
        sort(seg.begin(), seg.end());

/*         F(i,0,seg.size()){
 *             cout << seg[i].d << ' ' << seg[i].w << endl;
 *         }
 *         cout << R << endl;
 */
        double totalT = 0;
        F(i,0,seg.size()){
            double runT = seg[i].d / (seg[i].w + R);
            if (runT <= t){
                totalT += runT;
                t -= runT;
            }else{
                totalT += t;
                totalT += (seg[i].d - (seg[i].w + R) * t) / seg[i].w;
                t = 0;
            }
            //cout << totalT << endl;
        }
