...
int N;
int maxP = 102400000;
int price[1025], miss[1025];
map<pair<int,int>, int> record;
int dp(int i, int missed){
    if (i >= N){
        int p = i - (N-1);
        if (missed <= miss[p]) return 0;
        else return maxP;
    }
    pair<int,int> p = mp(i, missed);
    if (record.find(p) == record.end()) {
        record[p] = min(min(
                dp(i+i, missed+1) + dp(i+i+1, missed+1), 
                price[i] + dp(i+i, missed) + dp(i+i+1, missed)
                ), maxP);
    }
    return record[p];
}
