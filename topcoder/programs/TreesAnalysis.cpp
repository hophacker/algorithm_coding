...
class TreesAnalysis
{
    private:
        int n;
    public:
        int getMax(set<int> set1, set<int> set2){
            int a1 = 0, a2 = 0,  a3 = 0, a4 = 0;
            F(i,0,n){
                if (set1.find(i) == set1.end()){
                    if (set2.find(i) == set2.end()) a1++;
                    else a2++;
                }else{
                    if (set2.find(i) == set2.end()) a3++;
                    else a4++;
                }
            }
            return max(max(a1, a2), max(a3, a4));
        }
        set<int> dfs(int x, int fa, vector<set<int>>&val, vector<vector<int> >& edge){
            set<int> cnt;
            cnt.insert(x);
            F(i,0,edge[x].size()){
                int y = edge[x][i];
                if (y != fa){
                    set<int> ret = dfs(y, x, val, edge);
                    cnt.insert(ret.begin(), ret.end());
                }
            }
            val[x] = cnt;
            return cnt;
        ...
        {
            n = tree1.size() + 1;
            vector<vector<int> > edge1(n, vector<int>()), edge2(n, vector<int>());
            vector<set<int>> val1(n), val2(n);
            F(i,0,tree1.size()) 
                edge1[i].push_back(tree1[i]),
                edge1[tree1[i]].push_back(i);
            F(i,0,tree2.size()) 
                edge2[i].push_back(tree2[i]),
                edge2[tree2[i]].push_back(i);
