...
{
    private:
        vector<vector<bool> > ok, ok1;
        int n, m;
    public:
        void check(int t, vector<string> &score){
            F(i,0,n)
                F(j,i+1,n){
                    if (score[i][t] > score[j][t] && !ok[j][i]) return;
                }
            F(i,0,n)
                F(j,i+1,n){
                    if (score[i][t] > score[j][t]) ok1[j][i] = true;
                }
...
//            printV(result);
            n = result.size();
            m = score[0].size();
            ok = vector<vector<bool> >(n, vector<bool>(n, false));
            ok1 = vector<vector<bool> >(n, vector<bool>(n, false));
            F(i,0,n)
                F(j,i+1,n){
                    if (result[i] > result[j]){
//                        cout << "result: " << result[i] <<  ' ' << result[j] << endl;
                        ok[result[i]][result[j]] = true;
                    }
                }
            F(t,0,m){
                check(t, score);
            ...
            cout << (1LL << 1) << endl;
            return ok1 == ok? "Possible":"Impossible";
        }
