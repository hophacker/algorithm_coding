...
int n, d;
ll a[300009];
map<ll, vector<int> > ii;
int pos(int p, ll value){
    if (ii.find(value) == ii.end()) return n;
    else{
        vector<int>& v = ii[value];
...
void add(ll value, int p){
    if (ii.find(value) == ii.end()){
        ii[value] = vector<int>();
    }
    ii[value].push_back(p);
}
int main ( int argc, char *argv[] ) {
    ...
    int L = -1;
    F(i,0,n){
        int p = min(pos(i, a[i] + d), pos(i, a[i] - d));
        if (p != n){
//            cout << a[i] << ' ' << p << endl;
            cnt += (ll)(n-p) * (ll)(i-L);
            L = i;
