...
#include <cstring>
#include <algorithm>
#define max(a,b) (a<b?b:a) /*These four macros are used to judge if number i is between l and r.  (l,r): IN [l,r]: LINR
    [l,r): LIN
    (l,r]: INR
*/
...
int main(){
    ...
    for (int j = 0; j < m; j++){
        for (int i = 0; i < n; i++)
            M[i][1] = (s[i][j] == '1'? M[i][0]+1 : 0);

        memcpy(k, M[1], sizeof(int) * n);
        memcpy(M[0], M[1], sizeof(int) * n);
        sort(k, k+n);       
        int maxn = 0;

        for (int i = 0; i < n; i++){
            maxn = max(maxn, (n-i) * k[i]);
        }
        maxN = max(maxN, maxn); 
    }
    cout << maxN << endl;
