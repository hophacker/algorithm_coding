...
private:
    ...
    int n;
public:
    TreeNode *build(int pl, int il, int len){
        if (len <= 0) return NULL;
        TreeNode* cur = new TreeNode(pre[pl]);
        int i;
        for (i = 0; i < len; i++)
            if (in[il+i] == pre[pl]) break; 
        cur->left = build(pl+1, il, i);
        cur->right = build(pl+i+1, il+i+1, len-1-i);
        return cur;
    }
...
int main ( int argc, char *argv[] ) {
    ...
    int pre[] = {1,2,3};
    int in[] = {2,1,3};
    vector<int> PRE(pre, pre+3);
    vector<int> II(in, in+3);
    TreeNode* root = s.buildTree(PRE, II);
    printTree(root);
    return EXIT_SUCCESS;
