...
public:
    TreeNode *divide(ListNode* head, int n){
        ...
        int LNum = n/2;
        ListNode* L = head;
        for (int i = 0; i < LNum; i++) head = head->next;
        TreeNode* treehead = new TreeNode(head->val);
        treehead->left = divide(L, LNum);
        treehead->right = divide(head->next, n - LNum-1);
        return treehead;
    }
    TreeNode *sortedListToBST(ListNode *head) {
        int n = 0;
        ListNode* h = head;
        while(h != NULL) h = h->next, n++;
...
int main ( int argc, char *argv[] ) {
    ...
    printList(head);
    Solution s = Solution();
    TreeNode* treehead = s.sortedListToBST(head);
    printTree(treehead);
    return EXIT_SUCCESS;
}
