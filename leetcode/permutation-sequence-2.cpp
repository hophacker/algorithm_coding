...
typedef pair<int,int> ii; template<typename T,typename TT> ostream& operator<<(ostream &s,pair<T,TT> t) {return s<<"("<<t.first<<","<<t.second<<")";} template<typename T> ostream& operator<<(ostream &s,vector<T> t){F(i,0,SZ(t))s<<t[i]<<" ";return s; }
...
public:
    string getPermutation(int n, int k) {
        ...
        for (int i = n; i >= 1; i--){
            kinds /= i;
            int t = k/kinds; k %= kinds;
            
            res.push_back(used[t]);
            used.erase(used.begin() + t);
        }
        return res;
    }
};
