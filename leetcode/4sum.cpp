...
public:
    ...
    vector<vector<int> > fourSum(vector<int> &num, int target) {
        ...
        map<int, vector<vector<int> > > sum2;
        sort(num.begin(), num.end());
        int size = num.size();

        int minII = num[0] + num[1];
        int maxII = num[size-1] + num[size-2];
        int minIII = num[0] + num[1] + num[2];
        int maxIII = num[size-1] + num[size-2] + num[size-3];

        //cout << minII << ' ' << maxII << endl;
        for (ui i = 0; i < num.size(); i++){
            for (ui j = i+1; j < num.size(); j++) {
                int s2 = num[i] + num[j];

                if (s2 + minII > target) continue;
        ...
        mi last = sum2.end(); last--;
       
        for (mi i = sum2.begin(); i != sum2.end(); i++) {
            if (i->first + last->first < target) continue;
            for (mi j = i; j != sum2.end(); j++) {
                int s4 = i->first + j->first;
                if (s4 > target) break;
                if (s4 == target){
                    for (ui x = 0; x < i->second.size(); x++)
                        for (ui y = (i==j?x+1:0); y < j->second.size(); y++)
                            solve(i->second[x], j->second[y]);
                }

            }
        }
        for (ui i = 0; i < sum4.size(); i++) {
            sort(sum4[i].begin(), sum4[i].end());
            for (ui j = 0; j < 4; j++) sum4[i][j] = num[sum4[i][j]];
        }
        return sum4;
    }
};
