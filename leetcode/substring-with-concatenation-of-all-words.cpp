...
public:
    int find(string word){
        if (dict.find(word) == dict.end()) return -1;
        else return dict[word];
    }
    bool ok(int i){
        vector<int> good(num.size(), 0);
        for (int j = 0; j < n; j++){
            int k = pos[i];
            if (k == -1) return false;
            good[k]++;
            if (good[k] > num[k]) return false;
            i += wordLen;
        }
        return true;
    }
    vector<int> findSubstring(string S, vector<string> &L) {
        ...
        n = L.size();
        pos = vector<int>(S.length());
        for (int i = 0; i <= S.length() - wordLen; i++)
            pos[i] = find(S.substr(i, wordLen));

        for (int i = 0; i <= S.length() - totalLen; i++) if (ok(i))
            res.push_back(i);
        return res;
    }
};
 ...
 */
    words.push_back("a");
    vi res = s.findSubstring("a", words);
    printV(res);
    return EXIT_SUCCESS;
}
