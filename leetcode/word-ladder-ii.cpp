...
public:
    void cleardict(unordered_set<string>& dict, unordered_set<string>& list){
        for (auto i = list.begin(); i != dict.end(); i++)
            dict.erase(*i);
    }
    void expand(unordered_set<string>& dict, string str){
        string pre_str = str;
        for (int i = 0; i < str.length(); i++){
            char ct = str[i];
            for (char c = 'a'; c <= 'z'; c++) if (c != ct){
                str[i] = c;
                if (dict.find(str) != dict.end()){ 
                    pre[str].push_back(pre_str);
                    next_lev.insert(str);
                }
            }
            str[i] = ct;
    ...
    void out(string end){
        if (start == end){
            ...
            vector<string>& pres = pre[end];
            for (int i = 0; i < pres.size(); i++){
                path.push_back(pres[i]);
                out(pres[i]);
                path.pop_back();
            }
        }
