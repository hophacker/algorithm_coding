...
bool ans;
bool use[1000];
inline bool ok(char* w, char* s, int l){
	int p = 0;
	while(w[p]!=0 && s[l+p]!=0 && w[p]==s[l+p]) p++;
	if (w[p]==0 || s[l+p]==0) return true;
	return false;
}
int sstrcmp(char* w, char* res, int l){
	int p = 0;
	while(w[p]!=0 && res[l+p]!=0 && w[p]==res[l+p]) p++;
	if (w[p] == 0) return -1;
	if (res[l+p] == 0) return 1;
	return w[p]-res[l+p];
}
void search(int l1, int l2){
	...
	if (l1 == l2){
		...
		int p = pos[s[l1]-'a'];
		F(i,p,n)if (!use[i]){
			if (len[i]+l1>L || len[i]+l1 != L && len[i] == l2-l1 ) continue; //!!
			if (!ok(word[i], s, l1)) continue;
			if (ans && sstrcmp(word[i],res,l1)>0)
				break;
		...
		int p = pos[s[l2]-'a'];
		F(i,p,n)if (!use[i]){
			if (len[i]+l2>L || len[i]+l2 != L && len[i] == l1-l2) continue; //!!
			if (!ok(word[i], s, l2)) continue;
			if (ans && sstrcmp(word[i],res,l2)>0)
				break;
