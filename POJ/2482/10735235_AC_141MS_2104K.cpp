...
#define putI(n) printtf("%d\n", n)
#define getS(s) scanf("%s", s)
#define F(i,l,r) for (intt i=l;i<r;i++)
#define FE(i,l,r) for (intt i=l;i<=r;i++)
#define FF(i,r,l) for (intt i=r;i>l;i--)
#define FFE(i,r,l) for (intt i=r;i>=l;i--)
...
intt y[2*maxN+1];
intt sum[4*maxN+1];
intt maxSum[4*maxN+1];
void insert(intt ii, intt L, intt R, intt x, intt c){
	if (L<R){
		intt mid = (L+R)>>1;
	...
	if (L==R) maxSum[ii] = sum[ii];
		...
		maxSum[ii] = max(maxSum[ii], maxSum[2*ii+1]+sum[2*ii]);
	}
}
bool cmp(const intt a, const intt b){
	return a < b;
}
int main(){
