...
void search(int n, int x1, int x2, int y1, int y2){
	if (x2-x1+1>h || y2-y1+1>w) return;
	if (n == 5 || n == 7 || n==9){
		FE(i,x1,x2)
			FE(j,y1,y2)
				cMap[i-x1][j-y1] = map[i][j];
		switch(n){
			case 5: if (!check(x2-x1+1, y2-y1+1,4)) return;
			case 7: if (!check(x2-x1+1, y2-y1+1,3)) return;
			case 9: if (!check(x2-x1+1, y2-y1+1,2)) return;
		}
	}
	if (n == N+1){
		FE(i,x1,x2)
...
int main(){
	...
	if (h>=3 && w>=3){
		if (N==1) cout << 1 << endl;
			...
			map[10][10] = map[11][10] = map[12][10] = 1;
			xW[1] = 10, xW[2] = 11, xW[3] = 12;
			yW[1] = yW[2] = yW[3] = 10;
			search(4,10,12,10,10);
			map[12][10] = 0;
			map[11][11] = 1;
			xW[3] = yW[3] = 11;
			search(4,10,11,10,11);
			cout << total << endl;
		}
	}else{
		map[10][10] = 1;
