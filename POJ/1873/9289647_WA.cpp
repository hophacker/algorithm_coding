...
int main(){
	...
	for (int i = 1; i <= 15; i++)
		pow2[i] = pow2[i-1] * 2;
	scanf("%d", &n);
	double cutLen, leftValue, cutNum, extraWood, maxLeftValue, minCutNum, extra;
	int treeNum, minCutTree = 0, cases = 0;
	bool first = true;
	while(n){
		...
		cutLen = 0;
		for (int cutTree = 1; cutTree < pow2[n]-1; cutTree++){
			leftValue = 0;
			cutNum = 0;
			treeNum = 0;
			cutLen = 0;
			...
			extraWood = calcExtra(treeNum, p3, convex, cutLen);
			if (extraWood >= 0 && (minCutTree == 0 || (leftValue>maxLeftValue || zero(leftValue-maxLeftValue) && cutNum < minCutNum))){
				minCutNum = cutNum;
		...
		for (int i = 0; i < n; i++)
			if (minCutTree & pow2[i]) printf(" %d", i+1);
		printf("\n");
